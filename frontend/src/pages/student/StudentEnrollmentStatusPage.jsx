...
function StudentEnrollmentStatusPage() {
  ...
  if (!branchId || !courseId) {
    return (
      <div className="mx-auto max-w-lg rounded-2xl border border-slate-200 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900">
        ...
        <Link to="/student/internships" className="mt-4 inline-block text-sm font-semibold text-violet-600">
          Browse trainings
        </Link>
      </div>
    )
  ...
  if (!loading && !hasStudentCourseRecord(userId, branchId, courseId)) {
    return (
      <div className="mx-auto max-w-lg rounded-2xl border border-slate-200 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900">
        <p className="text-sm text-slate-600 dark:text-slate-400">You are not enrolled in this course yet.</p>
        <Link to="/services" className="mt-4 inline-block text-sm font-semibold text-violet-600">
          Browse programs
        </Link>
      </div>
    )
