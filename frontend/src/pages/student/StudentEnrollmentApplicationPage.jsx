...
import { buildEnrollmentStatusPath } from '../../utils/courseAccessRoutes.js'
import {
  buildStudentCourseNavLink,
  STUDENT_COURSE_WORKSPACE_PATH,
} from '../../components/nav/training-sphere/navConstants.js'
import { fetchCourseCompanyContact } from '../../utils/resolveCourseCompany.js'
import { fetchCourseTrainer, resolveCourseTrainer } from '../../utils/resolveCourseTrainer.js'
...
function StatusBanner({ onboarding, application, branchId, courseId }) {
      ...
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 dark:border-emerald-900/50 dark:bg-emerald-950/40">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <Link
              to={buildStudentCourseNavLink(STUDENT_COURSE_WORKSPACE_PATH, branchId, courseId)}
...
function StudentEnrollmentApplicationPage() {
  ...
  const { notify, token, userId, trainerName: studentName, email } = useOutletContext()
  const { role } = useAuth()
  ...
  const localMeta = useMemo(() => resolveCourseTrainer(branchId, courseId), [branchId, courseId])
  const [courseTrainer, setCourseTrainer] = useState(localMeta)
  const meta = courseTrainer.trainerEmail ? courseTrainer : localMeta
  ...
  const [companyContact, setCompanyContact] = useState({ companyEmail: '', companyName: '' })
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void fetchCourseCompanyContact(branchId, courseId).then((contact) => {
      if (!cancelled) setCompanyContact(contact)
    })
    return () => {
      cancelled = true
    }
  ...
  useEffect(() => {
    if (!branchId || !courseId) return
    let cancelled = false
    void fetchCourseTrainer(branchId, courseId).then((trainer) => {
      if (!cancelled && trainer?.trainerEmail) setCourseTrainer(trainer)
    })
    return () => {
      cancelled = true
    }
  ...
  if (!loading && !hasStudentCourseRecord(userId, branchId, courseId, email)) {
    return (
      <Navigate
  ...
  const showForm =
    !isApplicationFormComplete(application) &&
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      navigate(buildEnrollmentStatusPath(branchId, courseId, courseTitle), { replace: true })
    } catch (err) {
      ...
      <header>
        ...
        {companyContact.companyEmail ? (
          <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
            Company contact
            {companyContact.companyName ? `: ${companyContact.companyName}` : ''}
            {' · '}
            {companyContact.companyEmail}
          </p>
        ) : meta.trainerName ? (
            ...
            >
              <div>
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-500 disabled:opacity-60 dark:bg-violet-500"
              >
