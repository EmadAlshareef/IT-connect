...
function TrainingEnrollButton({
  ...
  const { isAuthenticated, role, userId, token, email, trainerName } = useAuth()
  ...
  const [seatStats, setSeatStats] = useState({ taken: 0, total: 20, isFull: false })

  const refresh = useCallback(() => {
    if (!userId) {
      setAccess({ enrolled: false, onboarding: 'none', application: null })
      setSeatStats(getCatalogTrainingSeatStats(branchId, trainingId))
      return
    }
  ...
  const handleEnroll = async (event) => {
    ...
    try {
      await startEnrollmentApplication(token, userId, email, trainerName, {
        branchId,
        courseId: trainingId,
      ...
      setError(err?.message || 'Could not complete enrollment. Try again.')
      setStatus('idle')
    }
  }
  ...
  if (!isAuthenticated) {
    ...
    return (
      ...
      <Link
        ...
        className={`inline-flex items-center justify-center rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-500 dark:bg-violet-500 dark:hover:bg-violet-400 ${className}`}
    ...
    return (
      ...
      <div className={`flex flex-col gap-2 ${className}`}>
        ...
        <Link
          ...
          to={buildStudentCourseNavLink(STUDENT_COURSE_WORKSPACE_PATH, branchId, trainingId)}

          onClick={(event) => event.stopPropagation()}

          className="text-center text-xs font-semibold text-violet-600 hover:text-violet-500 dark:text-violet-300"
    ...
    return (
      ...
      <div className={`flex flex-col gap-2 ${className}`}>
        ...
        <Link
          ...
          className={`inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 font-semibold text-white shadow-sm transition hover:bg-violet-500 dark:bg-violet-500 dark:hover:bg-violet-400 ${
  ...
  if (!access.enrolled && seatStats.isFull) {
    return (
      <div className={`flex flex-col gap-2 ${className}`}>
        <span className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">
          Training full ({seatStats.taken}/{seatStats.total} seats)
        </span>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          All seats are reserved for this program.
        </p>
      </div>
    )
  }
  ...
  return (
    ...
    <div className={`flex flex-col gap-1 ${className}`}>
      ...
      <button
        ...
        className={`inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 font-semibold text-white shadow-sm transition hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-70 dark:bg-violet-500 dark:hover:bg-violet-400 ${
