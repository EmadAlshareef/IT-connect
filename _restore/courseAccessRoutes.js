...
import { getCourseAccessState, listApprovedCatalogEnrollments } from './courseEnrollmentAccess.js'
import { hasCatalogEnrollmentRecord } from './trainingCatalogEnrollment.js'
...
export function resolveCourseContentRedirect(userId, pathname, searchParams) {
  ...
  if (!course) {
    ...
    const enrollments = userId
      ? (typeof window !== 'undefined'
          ? JSON.parse(localStorage.getItem(`ts-catalog-training-enrollments-${userId}`) || '[]')
          : [])
      : []
  ...
  if (!hasCatalogEnrollmentRecord(userId, course.branchId, course.courseId)) {
